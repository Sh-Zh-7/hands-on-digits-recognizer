...
def StoreParams(alphas, bs):
    import pickle
    with open("../params/alphas.svm", "wb") as f1, open("../params/bs.svm", "wb") as f2:
...
def LoadParams():
    import pickle
    with open("../params/alphas.svm", "rb") as f1, open("../params/bs.svm", "rb") as f2:
...
def TestRBF(k_tup=("rbf", 10)):
    data_set, labels = GetDataSetByDir("../data/train_set/")
    # Get b and alphas
    ...
    # Get test set error
    data_set, labels = GetDataSetByDir("../data/test_set/")
    class_sets = GetClassSet(labels)
    error_count = 0
    data_set = np.mat(data_set)
    labels = np.mat(labels).transpose()
